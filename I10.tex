...
After some time I realised that this whilst necessary, is not sufficient. At one point I called this``tactical'' 
quality -- meaning that it reflects the day to day concerns of the developer, which is separate from a 
``strategic'' quality, which is the quality that is perceived by the user throughout the software's operational 
lifetime. 
...
written a standard on it where it describes quality in terms of ``quality in use'' model and a ``product quality'' 
...
qualities are a class of requirements that are often called ``non-functional''. This means that they often avoided 
...
\begin{itemize}
	...
	\item ``Software has to be designed from the ground up to be secure.''
	\item ``Reliability engineering is weaved into the total development cycle.''
\end{itemize}
...
Peter pointed me to the ``Views and Beyond'' software architecture approach developed 
...
(i.e. reliability). 
	...
	\item A developer wants to know if she has ``broken the build'' within 30 minutes of 
...
but I keep coming back to Conways Law: ``organizations which design systems \hellip 
...
popularised by Fred Brooks in the ``Mythical Man Month''~\citep{brooks1995}. Conway also 
...
but his conclusion, two paragraphs later: ``Ways must be found to reward design managers for keeping 
